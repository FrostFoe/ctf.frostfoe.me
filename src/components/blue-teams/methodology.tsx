...
export default function Methodology() {
  const methodologyImage = PlaceHolderImages.find(
    (img) => img.id === "blue-team-learning",
  )!;

  return (
    <section className="w-full">
      <div className="rounded-lg border border-blue-500/20 bg-gradient-to-b from-blue-950/20 to-transparent p-8 md:p-12">
          ...
          <div className="w-full lg:w-1/2">
            ...
            <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
              {stats.map((stat) => (
                <Card
                  key={stat.label}
                  className="rounded-lg border border-blue-500/30 bg-black/30 p-4 text-center"
                >
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {features.map((feature) => (
          <a key={feature.title} href="#" className="group block h-full">
            <Card className="h-full transform rounded-lg border border-gray-800 bg-gray-900/50 p-6 transition-all duration-300 ease-in-out group-hover:-translate-y-2 group-hover:scale-105 group-hover:border-blue-500/30 group-hover:shadow-lg group-hover:shadow-blue-950/50">
              <CardHeader className="p-0">
                <Image
