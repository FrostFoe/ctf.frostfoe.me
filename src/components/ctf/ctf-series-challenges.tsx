...
export default function CtfSeriesChallenges({
  ...
  const difficultyColors: Record<string, string> = {
    ...
    "মাঝারি থেকে কঠিন": "bg-orange-900/20 text-orange-300 border border-orange-800",
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-white">সিরিজ অগ্রগতি</h3>
      ...
      {/* Challenges List */}
      <div className="space-y-3">
        <h3 className="text-lg font-bold text-white">চ্যালেঞ্জ সমূহ</h3>

        {sortedChallenges.length === 0 ? (
          <p className="text-slate-400 text-sm">কোন চ্যালেঞ্জ পাওয়া যায়নি</p>
        ) : (
          <div className="space-y-2">
            {sortedChallenges.map((challenge) => (
              ...
              >
                <div className="flex items-center gap-3 p-4 rounded-lg bg-slate-800/50 border border-slate-700 hover:border-slate-600 hover:bg-slate-800 transition-all group cursor-pointer">
                {/* Status Icon */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm text-slate-400 font-mono bg-slate-900 px-2 py-0.5 rounded">
                      #{challenge.seriesOrder}
                    </span>
                    <h4 className="font-semibold text-white group-hover:text-lime-400 transition-colors truncate">
                      {challenge.title}
                    </h4>
                  </div>
                ...
                <div className="flex-shrink-0">
                  {challenge.isCompleted ? (
                    <span className="text-xs font-bold text-lime-400 bg-lime-400/10 px-2 py-1 rounded border border-lime-800">
                      সমাধান করা
                    </span>
                  ) : (
                    <span className="text-xs font-bold text-slate-400 bg-slate-900 px-2 py-1 rounded border border-slate-700">
                      অমীমাংসিত
                    </span>
                  )}
                </div>
      ...
      {completedChallenges === totalChallenges && totalChallenges > 0 && (
        <div className="p-4 bg-lime-900/20 border border-lime-800 rounded-lg">
