...
export default function AdminChallengesPage() {
    ...
    try {
      ...
      if (eventsData && eventsData.length > 0) {
        ...
        for (const event of eventsData) {
          const eventChallenges = await getChallengesByEvent(event.id);
          allChallenges.push(...eventChallenges);
        }
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("আপনি কি এই চ্যালেঞ্জ ডিলিট করতে চান?")) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "চ্যালেঞ্জ ডিলিট করতে ব্যর্থ");
      }

      setChallenges(challenges.filter((c) => c.id !== id));
      alert("চ্যালেঞ্জ সফলভাবে ডিলিট হয়েছে");
    } catch (err) {
      console.error("Failed to delete challenge:", err);
      alert(
        err instanceof Error ? err.message : "চ্যালেঞ্জ ডিলিট করতে ব্যর্থ"
      );
    }
  };
  ...
  const filteredChallenges = challenges.filter((challenge) => {
    ...
    const matchesEvent =
      selectedEventId === null || challenge.event_id === selectedEventId;
