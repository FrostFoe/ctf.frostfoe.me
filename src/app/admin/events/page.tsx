...
export default function AdminEventsPage() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("আপনি কি এই ইভেন্ট ডিলিট করতে চান?")) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "ইভেন্ট ডিলিট ব্যর্থ");
      }

      setEvents(events.filter((e) => e.id !== id));
      alert("ইভেন্ট সফলভাবে ডিলিট হয়েছে");
    } catch (err: any) {
      console.error("Failed to delete event:", err);
      alert(`ইভেন্ট ডিলিট করতে ব্যর্থ: ${err?.message || 'Unknown error'}`);
    }
  };
                ...
                {filteredEvents.map((event) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-300">
                      {event.ctf_type || "jeopardy"}
                    </td>
                    <td className="px-6 py-4 text-slate-300">
                      {event.total_challenges || 0}
                    </td>
